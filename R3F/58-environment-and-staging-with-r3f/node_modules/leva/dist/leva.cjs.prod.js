...
var vectorPlugin = require('./vector-plugin-5405c78c.cjs.prod.js');
var v8n = require('v8n');
...
function getValuesForPaths(data, paths) {
  return Object.entries(vectorPlugin.pick(data, paths)).reduce((acc, [, {
    value,
    disabled,
    key
  }]) => {
...
function useDeepMemo(fn, deps) {
  ...
  return React.useMemo(fn, (_useCompareMemoize = useCompareMemoize(deps, true)) !== null && _useCompareMemoize !== void 0 ? _useCompareMemoize : []);
}
...
const Store = function Store() {
    ...
    store.setState(s => {
      ...
      Object.entries(newData).forEach(([path, newInputData]) => {
        ...
        if (!!input) {
          ...
          if (type !== input.type) {
            vectorPlugin.warn(vectorPlugin.LevaErrors.INPUT_TYPE_OVERRIDE, path, input.type, type);
          } else {
  ...
  const _getDataFromSchema = (schema, rootPath, mappedPaths) => {
    ...
    Object.entries(schema).forEach(([key, rawInput]) => {
      if (key === '') return vectorPlugin.warn(vectorPlugin.LevaErrors.EMPTY_KEY);
      ...
      if (rawInput.type === vectorPlugin.SpecialInputs.FOLDER) {
        ...
        if (normalizedInput) {
          ...
          vectorPlugin.warn(vectorPlugin.LevaErrors.UNKNOWN_INPUT, newPath, rawInput);
        }
      }
    });
    return data;
...
function TitleWithFilter({
  ...
  React.useEffect(() => {
    ...
    if (filterShown) (_inputRef$current = inputRef.current) === null || _inputRef$current === void 0 || _inputRef$current.focus();else (_inputRef$current2 = inputRef.current) === null || _inputRef$current2 === void 0 || _inputRef$current2.blur();
  }, [filterShown]);
  const bind = vectorPlugin.useDrag(({
    offset: [x, y],
    first,
...
const LevaCore = React__default["default"].memo(({
  ...
  return React__default["default"].createElement(RadixTooltip__namespace.Provider, null, React__default["default"].createElement(vectorPlugin.PanelSettingsContext.Provider, {
    value: {
    ...
    if (isGlobalPanel && !rootInitialized) {
      if (!rootEl) {
        ...
        if (document.body) {
          ...
          client.createRoot(rootEl).render(React__default["default"].createElement(Leva, {
            isRoot: true
          }));
        }
      }
...
exports.LevaInputs = vectorPlugin.LevaInputs;
