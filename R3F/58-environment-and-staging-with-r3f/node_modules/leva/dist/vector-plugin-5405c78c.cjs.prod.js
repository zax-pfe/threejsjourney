...
let LevaErrors = function (LevaErrors) {
...
function evaluate(expr) {
  ...
  if (muldiv.test(expr)) {
    return evaluate(expr.replace(muldiv, (_, a, op, b) => op === '*' ? (Number(a) * Number(b)).toString() : Number(b) === 0 ? (() => {
      throw new Error('Division by zero');
    })() : (Number(a) / Number(b)).toString()));
  }
  if (addsub.test(expr)) {
    return evaluate(expr.replace(addsub, (_, a, op, b) => op === '+' ? (Number(a) + Number(b)).toString() : (Number(a) - Number(b)).toString()));
  }
  return Number(expr);
}
...
let SpecialInputs = function (SpecialInputs) {
...
let LevaInputs = function (LevaInputs) {
...
stitches.css;
const createTheme = stitches.createTheme;
...
function RawLabel(props) {
  ...
  const htmlForProp = props.htmlFor || (id ? id : undefined);
  const titleProp = !hint && typeof props.children === 'string' ? props.children : undefined;
  return React__default["default"].createElement(React__default["default"].Fragment, null, optional && React__default["default"].createElement(OptionalToggle, null), hint !== undefined ? React__default["default"].createElement(RadixTooltip__namespace.Root, null, React__default["default"].createElement(RadixTooltip__namespace.Trigger, {
    asChild: true
  }, React__default["default"].createElement(StyledLabel, _extends({
    htmlFor: htmlForProp
  }, props))), React__default["default"].createElement(RadixTooltip__namespace.Content, {
    side: "top",
    sideOffset: 2
  }, React__default["default"].createElement(StyledToolTipContent, null, hint, React__default["default"].createElement(ToolTipArrow, null)))) : React__default["default"].createElement(StyledLabel, _extends({
    htmlFor: htmlForProp,
    title: titleProp
  }, props)));
}
...
exports.Label = Label;
exports.LevaErrors = LevaErrors;
exports.LevaInputs = LevaInputs;
...
exports.SpecialInputs = SpecialInputs;
